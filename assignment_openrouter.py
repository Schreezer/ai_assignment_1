...
def setup_api() -> OpenAI:
    """Set up the OpenAI client with OpenRouter configuration."""
...
def get_model_answer(client: OpenAI, query_prompt: str) -> str:
    """Get answer from OpenRouter API with retries and proper rate limiting."""
    try:
        # Apply rate limiting before making the call
        rate_limiter.wait_if_needed()
        
        logger.info(f"Sending query to OpenRouter API: {query_prompt}")
    ...
    "json_schema": {
      ...
      "schema": {
        ...
        logger.info(f"Received response from OpenRouter API: {full_response}")
...
def process_wsc_example(client: OpenAI, example: dict) -> Dict:
    ...
    model_response = get_model_answer(client, query)
...
def main():
    ...
    try:
        ...
        eval_results = evaluate_wsc(
            client, wsc273_dataset, num_examples, delay_between_queries=min_delay
        )
